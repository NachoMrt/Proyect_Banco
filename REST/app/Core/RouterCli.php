    ...
    */
    public function route($method, $uri, $data=null) {
        ...
        */
        ...
        if($parts[0] != 'cliente') {
            http_response_code(404);  // Si la primera parte de la URL no es 'Clientes', devuelve 404.
        ...
        // Si hay un segundo segmento en la URL (Clientes/1), lo guarda en $id.
        //Si no existe (Clientes), $id = null.
        $id = $parts[1] ?? null;

        // Según el método HTTP, decide qué acción ejecutar en el controlador:
        switch($method) {
            case 'GET':
                $id ? $this->controller->show($id) : $this->controller->index(); // Si hay ID → muestra un Cliente específico
                                                                                    // Si no → devuelve todos los Clientes
                break;
            case 'POST':
...
Router.php es como el centro de control de la API:
    ...
    Limpia y separa la ruta (Clientes, Clientes/1)
